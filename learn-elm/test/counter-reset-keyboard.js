...
function update(action, model) {
  // Update function takes the current state
  switch (
    action // and an action (String) runs a switch
  ) {
    case "inc":
      return model + 1; // add 1 to the model
    case "dec":
      return model - 1; // subtract 1 from model
    case "reset":
      return 0; // reset state to 0 (Zero) git.io/v9KJk
    default:
      return model; // if no action, return curent state.
